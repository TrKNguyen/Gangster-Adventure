...
bool Pic::loadIMG(std::string path, SDL_Renderer* screen, int _w, int _h, int animation_frame)
{
    SDL_Texture* new_texture = NULL;
    SDL_Surface* load_surface = IMG_Load(path.c_str());
    if(load_surface != NULL)
    {
        // xoa phong voi mau phong la 3 so cuoi 
...
bool Pic::loadText(std::string text, SDL_Renderer* screen, int _w, int _h, TTF_Font* Font, SDL_Color textColor)
{
    SDL_Texture* new_texture = NULL;
    SDL_Surface* load_surface = TTF_RenderText_Solid( Font, text.c_str(), textColor );
    // font la font , text.c_str() la noi dung, textColor la mau chu
    if(load_surface != NULL)
    {
        new_texture = SDL_CreateTextureFromSurface(screen, load_surface);
        if(new_texture != NULL)
...
void Pic::Render(SDL_Renderer* screen, int x, int y, SDL_Rect* RectClip)
{
